...
# Gradle 8.13 supports Java up to 23; fall back to Zulu 21 on macOS when the
# default `java` is newer (e.g. Java 25) so `buildCustomApkDebug` succeeds.
if [[ -z "${JAVA_HOME:-}" || "$(java -version 2>&1 | head -1)" =~ \"2[4-9]| ]]; then
...
# Android SDK location is required by Gradle.
